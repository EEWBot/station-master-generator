...
//!
//! A coordinate is not a point but a quantization cell: `43.17` from a
//! two-decimal source means the true latitude lies in `[43.165, 43.175)`. Treating
...
///
/// `43.17 - 0.01 / 2` is not exactly `43.165` in binary floating point, so the
...
/// `43.1714 / 0.0001` and `43.17 / 0.01` are the same place, while a genuine move
...
mod tests {
    ...
    fn coarse_cell_contains_fine_coordinate() {
        let coarse = loc(43.17, 141.32, 0.01);
        let fine = loc(43.1714, 141.3156, 0.0001);
    ...
    fn cell_is_half_open() {
        ...
        assert!(same_point(&coarse, &loc(43.165, 141.32, 0.0001)));
        // Upper bound belongs to the next cell.
        assert!(!same_point(&coarse, &loc(43.175, 141.32, 0.0001)));
    }

    #[test]
    fn longitude_is_checked_too() {
        let coarse = loc(43.17, 141.32, 0.01);
        assert!(!same_point(&coarse, &loc(43.1714, 141.3256, 0.0001)));
    }

    #[test]
    fn a_real_move_leaves_the_cell() {
        let before = loc(43.1714, 141.3156, 0.0001);
        let after = loc(43.2300, 141.4100, 0.0001);
        assert!(!same_point(&before, &after));
    }

    #[test]
    fn identical_fine_coordinates_are_the_same_point() {
        let a = loc(43.1714, 141.3156, 0.0001);
    ...
    fn adjacent_fine_coordinates_are_different_points() {
        let a = loc(43.1714, 141.3156, 0.0001);
        let b = loc(43.1715, 141.3156, 0.0001);
